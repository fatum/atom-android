...
class ErrorTracker {
	...
	protected void trackError(String errorString) {
		if (IsaConfig.getInstance(mContext)
		             .isErrorReportingEnabled()) {
			String stream = IsaConfig.getInstance(mContext)
			                         .getSdkErrorStream();
			String authKey = IsaConfig.getInstance(mContext)
			                          .getSdkErrorStreamAuthKey();

			Logger.log(TAG, "TRACKING ERROR TO: " + stream + " / " + authKey, Logger.SDK_DEBUG);

			IronSourceAtomTracker sdkTracker = IronSourceAtomFactory.getInstance(mContext)
			                                                        .newTracker(authKey);
			...
			try {
				...
				errorReport.put("timestamp", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH).format(Calendar.getInstance()
				                                                                                                        .getTime()));
				errorReport.put("sdk_version", BuildConfig.VERSION_NAME);

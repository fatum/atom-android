...
import io.ironsourceatom.sdk.DbAdapter.DatabaseHandler;
...
public class DbAdapterTest {
	...
	public void getTables() {
		...
		SQLiteDatabase db = mock(SQLiteDatabase.class);
		when(db.query(anyString(), any(String[].class), anyString(), any(String[].class), anyString(), anyString(), anyString())).thenReturn(cursor);
		when(handler.getReadableDatabase()).thenReturn(db);
		// 3 iterations
	...
	public void addEvent1() {
		...
		verify(db, times(1)).insert(eq(DbAdapter.REPORTS_TABLE), isNull(String.class), any(ContentValues.class));
	}
	...
	public void addEvent2() {
		...
		verify(db, times(1)).insert(eq(DbAdapter.REPORTS_TABLE), isNull(String.class), any(ContentValues.class));
		verify(db, times(1)).insertWithOnConflict(eq(DbAdapter.TABLES_TABLE), isNull(String.class), any(ContentValues.class), eq(SQLiteDatabase.CONFLICT_IGNORE));
	}
